...
def define_community():
    if request.method == 'POST':
        ...
        if community_id:
            ...
            flash(
                f'Congratulations! Your community "{community_name}" has been created successfully.',
                'success')
            return redirect(url_for('dashboard'))
        else:
            if error:
...
def dashboard():
    ...
    # Get member count
    member_count = len(get_community_members(current_user.community_id))
...
def post_alert():
    ...
    if request.method == 'POST':
        ...
        alert_id, error = create_alert(current_user.community_id,
